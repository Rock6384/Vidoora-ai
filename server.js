...
async function runVideoPipeline(jobId, userPrompt) {
  ...
  try {
    ...
    // Call Replicate API (Minimax Video-01 Model for fast high quality video)
    ...
    if (!response.ok || prediction.error) {
      job.status = "failed";
      job.error = prediction.error || "Replicate prediction failed to start";
      return;
    }

    // Poll Replicate until video is rendered
    ...
    while (!completed) {
      ...
      if (pollData.status === "processing") {
        job.progress = Math.min((job.progress || 25) + 8, 90);
